...
import torch # Добавляем для проверки CUDA
from sentence_transformers import CrossEncoder # Добавляем для реранкера
...
SIMILARITY_TOP_K = 12
# Добавляем константы для реранкера
RERANKER_MODEL_NAME = 'DiTy/cross-encoder-russian-msmarco'
RERANKER_TOP_N = SIMILARITY_TOP_K # Сколько узлов оставляем после реранкинга (берем то же значение, что и у LLM)
INITIAL_RETRIEVAL_TOP_K = 25 # Сколько узлов извлекаем изначально для реранкера
...
reranker_model = None # Глобальная переменная для реранкера
...
def initialize_index():
    global query_engine, global_index, reranker_model # Добавляем реранкер в глобальные переменные
    print("Инициализация/Загрузка индекса и реранкера...")
    ...
    llm = Ollama(
        ...
        context_window=100000,  # Установлено согласно возможностям модели пользователя (100k/128k)
        request_timeout=300.0 # Увеличиваем таймаут до 300 секунд (5 минут)
    )
    print(f"Используется LLM: Ollama (Модель: {OLLAMA_LLM_MODEL_NAME}, URL: {OLLAMA_BASE_URL}, Timeout: 300s)")
    ...
    if reranker_model is None:
        try:
            ...
            print(f"Инициализация реранкера ({RERANKER_MODEL_NAME}) на устройстве: {device}")
            # max_length=512 это стандарт для многих CrossEncoder'ов, можно оставить
            reranker_model = CrossEncoder(RERANKER_MODEL_NAME, max_length=512, device=device)
            print("Реранкер успешно инициализирован.")
        except Exception as e:
            print(f"Ошибка при инициализации реранкера: {e}. Реранкинг будет отключен.")
            reranker_model = None # Отключаем реранкер при ошибке
    # -------------------------------
    ...
    # Создаем query engine как и раньше (он не знает про реранкер, реранкинг будет применен позже)
    query_engine = index.as_query_engine(
        llm=llm,
        similarity_top_k=INITIAL_RETRIEVAL_TOP_K # Используем увеличенный K для начального поиска!
    )
    # Заменяем SIMILARITY_TOP_K на INITIAL_RETRIEVAL_TOP_K в сообщении
    print(f"Query engine готов (initial_similarity_top_k={INITIAL_RETRIEVAL_TOP_K}). Реранкинг будет применен отдельно.") 
    return query_engine # Возвращаем только query_engine
# -----------------------------------------------------------

# --- Функция запроса с уточненным промптом ---
def query_case(case_description: str, pension_type: Optional[str] = None, disability_info: Optional[dict] = None) -> str:
    """
    Выполняет RAG-запрос, создавая ретриверы, объединяя результаты,
    применяя реранкинг (если доступен) и генерируя ответ LLM.
    Учитывает тип пенсии и данные об инвалидности.
    """
    engine = get_query_engine() # Инициализирует engine, global_index и reranker_model
    if not engine or not global_index:
        return "Ошибка: Query engine или Index не инициализированы."
    
    # Используем глобальную переменную реранкера, инициализированную в get_query_engine/initialize_index
    global reranker_model 

    # --- Логируем полученные доп. параметры ---
    # <<< Логируем только если pension_type передан >>>
    if pension_type:
        print(f"RAG Query: Тип пенсии={pension_type}")
    else:
        print(f"RAG Query: Тип пенсии не указан (используется общий поиск).")
    # ----------------------------------------
    if disability_info:
        print(f"RAG Query: Данные об инвалидности={disability_info}")
    # -------------------------------------------
    ...
    try:
        print(f"Выполняется поиск по запросу: '{case_description[:100]}...'"
              f" (Initial K={INITIAL_RETRIEVAL_TOP_K}, Rerank Top N={RERANKER_TOP_N if reranker_model else 'N/A'}, Reranker: {'ON' if reranker_model else 'OFF'})")

        target_filters = None
        # <<< Применяем фильтры ТОЛЬКО если pension_type передан и распознан >>>
        if pension_type:
            if pension_type == 'retirement_standard' and "досрочн" not in case_description.lower():
                print("Обнаружен запрос 'пенсия по старости'. Применяем фильтр для Статьи 8 ФЗ-400.")
                target_filters = MetadataFilters(
                    filters=[MetadataFilter(key="article", value="Статья 8"), MetadataFilter(key="file_name", value="fz400.pdf")] # Уточняем закон
                )
            elif pension_type == 'disability_social':
                print("Обнаружен запрос 'социальная пенсия по инвалидности'. Ищем Статью 11 ФЗ-166.")
                target_filters = MetadataFilters(
                    filters=[MetadataFilter(key="article", value="Статья 11"), MetadataFilter(key="file_name", value="fz166.pdf")] # Пример!
                )
            # Добавить другие типы пенсий здесь...
        # ----------------------------------------------------------------------

        print(f"Создание ретривера для основного поиска (k={INITIAL_RETRIEVAL_TOP_K})...")
        # Используем INITIAL_RETRIEVAL_TOP_K для основного ретривера
        general_retriever = global_index.as_retriever(similarity_top_k=INITIAL_RETRIEVAL_TOP_K) 
        general_nodes = general_retriever.retrieve(case_description)
        print(f"Основной поиск завершен, найдено {len(general_nodes)} узлов-кандидатов.")

        filtered_nodes = []
        if target_filters:
            print(f"Создание ретривера для поиска с фильтром (k={INITIAL_RETRIEVAL_TOP_K}): {target_filters}")
            # Используем INITIAL_RETRIEVAL_TOP_K и для фильтрованного ретривера
            ...
            print(f"Найдено {len(filtered_nodes)} узлов-кандидатов с фильтром.")

        # --- Объединяем ВСЕХ кандидатов перед реранкингом --- 
        all_candidate_nodes_dict = {}
        # Сначала добавляем отфильтрованные (они могут иметь бОльший приоритет, если оценка реранкера будет одинаковой)
        for node_with_score in filtered_nodes:
            if node_with_score and node_with_score.node and hasattr(node_with_score.node, 'id_'):
                all_candidate_nodes_dict[node_with_score.node.id_] = node_with_score
        # Добавляем общие, не перезаписывая уже добавленные отфильтрованные
        for node_with_score in general_nodes:
            if node_with_score and node_with_score.node and hasattr(node_with_score.node, 'id_'):
                if node_with_score.node.id_ not in all_candidate_nodes_dict:
                    all_candidate_nodes_dict[node_with_score.node.id_] = node_with_score
        
        all_candidate_nodes_list = list(all_candidate_nodes_dict.values())
        print(f"Всего уникальных узлов-кандидатов для реранкинга: {len(all_candidate_nodes_list)}")
        # -----------------------------------------------------

        # --- Применяем реранкинг, если модель доступна --- 
        ranked_nodes = []
        if reranker_model and all_candidate_nodes_list:
            print(f"Применение реранкера ({RERANKER_MODEL_NAME})...")
            # Формируем пары [query, passage_text] для реранкера
            query_doc_pairs = []
            node_map = {} # Словарь для быстрого доступа к узлу по его тексту
            for node_with_score in all_candidate_nodes_list:
                if node_with_score and node_with_score.node and hasattr(node_with_score.node, 'get_content'):
                    passage_text = node_with_score.node.get_content()
                    query_doc_pairs.append([case_description, passage_text])
                    node_map[passage_text] = node_with_score # Сохраняем узел
            
            # Получаем оценки реранкера
            rerank_scores = reranker_model.predict(query_doc_pairs, convert_to_numpy=True) # Используем predict
            
            # Создаем новый список узлов с оценками реранкера
            reranked_results = []
            texts_in_pairs = [pair[1] for pair in query_doc_pairs] # Получаем тексты в том же порядке
            for i, passage_text in enumerate(texts_in_pairs):
                 original_node_with_score = node_map.get(passage_text)
                 if original_node_with_score:
                     # Создаем новый NodeWithScore, заменяя score на оценку реранкера
                     reranked_results.append(NodeWithScore(
                         node=original_node_with_score.node,
                         score=float(rerank_scores[i]) # Преобразуем в float
                     ))
                 else:
                      print(f"Предупреждение: Не найден узел для текста при реранкинге: {passage_text[:50]}...")

            # Сортируем по оценке реранкера (по убыванию)
            ranked_nodes = sorted(reranked_results, key=lambda x: x.score, reverse=True)
            print(f"Реранкинг завершен. Лучший score: {ranked_nodes[0].score:.4f}" if ranked_nodes else "(нет результатов после реранкинга)")
        else:
            # Если реранкер недоступен или нет кандидатов, используем исходную сортировку по score из ретривера
            print("Реранкер недоступен или нет кандидатов. Используется сортировка по исходному score.")
            ranked_nodes = sorted(all_candidate_nodes_list, key=lambda n: n.score if hasattr(n, 'score') else -1, reverse=True)
        # ---------------------------------------------------

        # --- Выбираем топ N узлов после реранкинга (или исходной сортировки) ---
        final_nodes_for_prompt = ranked_nodes[:RERANKER_TOP_N]
        print(f"Отобрано топ-{len(final_nodes_for_prompt)} узлов для передачи в LLM.")
        # ----------------------------------------------------------------------

        # --- Формируем контекст для LLM (код остается прежним, но работает с final_nodes_for_prompt) ---
        ...
        if final_nodes_for_prompt:
             for i, node_with_score in enumerate(final_nodes_for_prompt):
                 if node_with_score and node_with_score.node and hasattr(node_with_score.node, 'metadata'):
                     ...
                     # Сначала получаем текст узла
                     text_preview = node.get_content() if hasattr(node, 'get_content') else "[Содержимое недоступно]"
                     # Теперь используем text_preview в print
                     print(f"Фрагмент {i+1} (Score: {score_str}, Статья: {metadata.get('article', 'N/A')}, Пункт: {metadata.get('point', 'N/A')}, Заголовок: {metadata.get('header', 'N/A')}):\n{text_preview}\n\n")
                     print(f"Начало: {text_preview[:150]}...")
                     if len(text_preview) > 300: print(f"...Конец: ...{text_preview[-150:]}")
                     print("---")
                     context_for_prompt += f"Фрагмент {i+1} (Score: {score_str}, Статья: {metadata.get('article', 'N/A')}, Пункт: {metadata.get('point', 'N/A')}, Заголовок: {metadata.get('header', 'N/A')}):\n{text_preview}\n\n"
                 else:
    ...
    # --- Формируем структурированное описание инвалидности для промпта ---
    disability_text = ""
    if disability_info:
        group_map = {"1": "I", "2": "II", "3": "III", "child": "Ребенок-инвалид"}
        group_str = group_map.get(disability_info.get('group'), disability_info.get('group'))
        date_str = disability_info.get('date', 'Не указана')
        cert_str = disability_info.get('cert_number')
        disability_text = f"\nДанные об инвалидности: Группа: {group_str}, Дата установления: {date_str}."
        if cert_str:
            disability_text += f" Номер справки МСЭ: {cert_str}."
    # ---------------------------------------------------------------------

    # --- Обновленный Промпт --- 
    # <<< Добавляем обработку случая, когда pension_type не задан >>>
    pension_type_map = {
        'retirement_standard': 'Страховая по старости (общий случай)',
        'disability_social': 'Социальная по инвалидности',
    }
    pension_type_text = pension_type_map.get(pension_type, pension_type if pension_type else "(тип не указан)") # Используем значение или заглушку
    # -----------------------------------------------------------

    prompt = (
        f"Ты — высококвалифицированный эксперт по пенсионному законодательству РФ (ФЗ-400, ФЗ-166 и др.). Твоя задача — провести детальный анализ описания пенсионного дела для назначения **{pension_type_text}**. \n"
        ...
        f"-----------------------------------------\n"
        f"{case_description}"
        f"{disability_text}\n" # <<< Вставляем форматированный текст об инвалидности
        f"-----------------------------------------\n\n"
        f"Фрагменты закона для анализа:\n"
        f"------------------------------------\n"
        f"{context_for_prompt}"
        f"------------------------------------\n\n"
        f"**Инструкции по анализу (с учетом типа пенсии {pension_type_text}):**\n"
        f"1.  **Проверь соответствие:** Последовательно проанализируй ключевые параметры дела (возраст, стаж, ИПК, группа инвалидности и т.д., в зависимости от релевантности для типа **{pension_type_text}**) на соответствие требованиям, изложенным в предоставленных фрагментах. Обрати особое внимание на условия для **{pension_type_text}**. \n"
        f"    *Для пенсии по старости (если это она):* Если в контексте есть фрагменты из Статьи 8 ФЗ-400, обязательно проверь соответствие возраста, страхового стажа (обычно не менее 15 лет) и ИПК (обычно не менее 30). \n"
        f"    *Для социальной пенсии по инвалидности (если это она):* Если есть фрагменты из ФЗ-166, проверь условия назначения для соответствующей группы инвалидности.\n"
        f"2.  **Выяви проблемы:** Четко укажи на любые несоответствия, проблемы или недостающую информацию для типа **{pension_type_text}**, основываясь *только* на тексте фрагментов. Если для какого-то аспекта дела во фрагментах нет информации, прямо скажи об этом.\n"
        f"3.  **Цитируй точно:** При ссылке на закон **обязательно указывай номер Статьи и/или Пункта**, используя информацию из метаданных фрагмента (например: \"согласно Статье 30 Пункту 5 ФЗ-400\", \"в соответствии со Статьей 11 ФЗ-166\"). \n"
        ...
        f"5.  **Структурируй ответ:** Сначала изложи анализ соответствия основным условиям для **{pension_type_text}**, затем — анализ особых условий или проблем. Заверши ответ кратким итоговым резюме.\n\n"
        # <<< Просим LLM дать явный вердикт в конце для упрощения анализа статуса
        f"**ВАЖНО: В самом конце ответа напиши одну из фраз: 'ИТОГ: СООТВЕТСТВУЕТ' или 'ИТОГ: НЕ СООТВЕТСТВУЕТ'.**\n\n"
        f"**Результат анализа:**\n"
     )
    # --------------------------

    # --- Используем ИСХОДНЫЙ query_engine для синтеза ответа --- 
    # Мы передаем кастомный промпт, он не будет сам делать retrieval заново
    print(f"Отправка запроса в LLM (через engine.query): {prompt[:400]}...")
    try:
        # Передаем наш детальный промпт в engine.query()
        # LLM внутри query_engine получит этот промпт и сгенерирует ответ
        response = engine.query(prompt) 
        response_text = str(response)
    except Exception as e:
        # Возвращаем обработку ошибок LLM
        print(f"Ошибка при выполнении запроса к Query Engine (LLM): {e}")
        import traceback
        traceback.print_exc()
        response_text = f"Ошибка при обработке запроса LLM: {e}"
    # -----------------------------------------------------------

    print(f"Ответ LLM (начало): {response_text[:250]}...")
...
if __name__ == '__main__':
    ...
    if test_engine:
        ...
        result = query_case(test_query, 'retirement_standard')
        print(f"\nРезультат анализа:\n{result}")
    else:
