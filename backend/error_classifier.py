...
class ErrorClassifier:
    ...
    def check_overlaps(self, records):
        periods = [(datetime.strptime(r["start_date"], "%Y-%m-%d"), 
                    datetime.strptime(r["end_date"], "%Y-%m-%d")) for r in records]
        periods.sort()
    ...
    def classify_errors(self, case_data):
        ...
        try:
            ...
            final_error_codes = set(errors)
            if not required_docs.issubset(set(case_data["documents"])):
                final_error_codes.add("E004")
            
            elapsed_time = time.time() - start_time
            logger.info(f"Классификация завершена за {elapsed_time:.2f} сек. Ошибки: {list(final_error_codes)}")
            if elapsed_time > 3:
                logger.warning(f"Время классификации превышает 3 сек: {elapsed_time:.2f} сек")
            
            # Формируем список словарей с полной информацией об ошибках
            error_details_list = []
            for code in final_error_codes:
                details = self.error_definitions.get(code, {})
                error_details_list.append({
                    "code": code,
                    "description": details.get("description", "Неизвестная ошибка"),
                    "law": details.get("law", "-"),
                    "recommendation": details.get("recommendation", "-")
                })
            
            return error_details_list # Возвращаем список словарей
        except Exception as e:
...
if __name__ == "__main__":
    ...
    case_data = {
        "personal_data": {
            ...
            "name_change_info": {"old_full_name": "Петров Петр Петрович", "date_changed": "2023-05-15"},
            "dependents": 1
        },
        "work_experience": {
            "total_years": 10,
            "records": [
                {"organization": "ООО Ромашка", "start_date": "2000-01-01", "end_date": "2010-01-01", "position": "Инженер", "special_conditions": True},
                {"organization": "АО Лютик", "start_date": "2005-02-01", "end_date": "2015-02-01", "position": "Менеджер", "special_conditions": False}
            ]
        },
        "pension_points": 25,
